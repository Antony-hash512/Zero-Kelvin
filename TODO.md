...
- [ ] **Подготовка данных:**
    - [ ] Функции генерации `list.yaml` в нужном формате на основе полученных аргументов. (unit-тесты -> реализация)
    - [ ] Функции для создания структуры каталогов в `to_restore` через биндинги. (unit(mock)-тесты -> реализация)
- [ ] **Упаковка:**
    - [ ] Оркестрация вызова `squash_manager-rs create`. (unit-тесты -> реализация)
    - [ ] Реализация основных shell-тестов в `tests/06_freeze.bats` для проверки корректности упаковки.
    - [ ] Поддержка режимов `plain` и `LUKS` (путём передачи вызова squash_manager-rs с передачей туда аргрументов и флагов). (unit-тесты -> реализация)
    - [ ] Реализация чтения списка целей из файла (`-r` / `--read`) (shell/unit-тесты -> реализация).
- [ ] **Cleanup:** Корректное удаление временных каталогов. (shell/unit-тесты -> реализация)
- [ ] Релизация защиты  от случайного удаления пользовательских данных, при попытках почистеть кеш во время работы ZKS:
    - [ ] Реализация песочницы для shell-тестов и реализация самих тестов.
