...
- [ ] **Тесты (Red stage):**
    - [ ] Unit-тесты в коде Rust
    - [ ] Shell-тесты в `tests/09_check.bats`:
- [ ] **Подготовка и Маппинг:**
    - [ ] Повторное использование `UnmountGuard` и логики монтирования из этапа 7.
    - [ ] Реализация итератора сопоставления путей архива (`to_restore/ID`) с реальной файловой системой.
- [ ] **Алгоритмы проверки:**
    - [ ] **Fast Check (Metadata):** Сравнение типа данных (является ли путь файлом, директорией или ссылкой) и размера в байтах.
    - [ ] **Deep Check (`--use-cmp`):** Реализация побайтового сравнения через внутренний буферизированный поток в Rust (без спавна внешних процессов `cmp`).
