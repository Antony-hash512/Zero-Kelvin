...
- [ ] **Интеграционные shell-тесты**
    - [ ] Реализовать тесты для `squash_manager-rs mount` (без прав рута т.к под капотом должен использоваться squashfuse)
    - [ ] Реализовать тесты для `squash_manager-rs mount` с автоматической генерацией имени каталога монтирования, если он не указан
    - [ ] Реализовать тесты для `squash_manager-rs umount`
    - [ ] Реализовать тест(ы) для `squash_manager-rs umount` с указанием образа вместо каталога монтирования
- [ ] **Код на Rust**
    - [ ] Реализовать проверку прав суперпользователя (через `utils.rs`):
        -  [ ] Юнит-тесты
        -  [ ] Реализация функционала
    - [ ] Реализовать дополнительные юнит-тесты для `squash_manager-rs mount`/`squash_manager-rs umount` (если актуально)
    - [ ] Реализовать успешное прохождение всех тестов для `squash_manager-rs mount`/`squash_manager-rs umount`  
    - [ ] Обновить справочную информацию для `squash_manager-rs umount` описав новые возможности:
        - [ ] автоматическая генерация имени каталога монтирования для `squash_manager-rs mount`
        - [ ] указания образа вместо каталога его монтирования для `squash_manager-rs umount`
