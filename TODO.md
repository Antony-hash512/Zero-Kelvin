...
- [ ] **Базовая структура проекта:**
    - [ ] Создать файлы модулей (`lib.rs`, `engine.rs` и др.) согласно структуре в GEMINI.md.
    - [ ] Создать заглушки для бинарников `bin/zks-rs.rs` и `bin/squash_manager-rs.rs`.
    - [ ] Реализовать флаги `-h`/`--help` и `-V`/`--version` (автоматически через clap).
- [ ] **CLI Парсинг (Clap):**
    - [ ] Реализовать структуру аргументов для `squash_manager-rs` (create, mount, umount).
    - [ ] Реализовать структуру аргументов для `zks-rs` (freeze, unfreeze, check).
    - [ ] Настроить инициализацию `env_logger` в точках входа.
...
- [ ] **Команда `create`:**
    ...
    - [ ] Портирование реализации trim для образки пустого хвоста файла в режиме LUKS
