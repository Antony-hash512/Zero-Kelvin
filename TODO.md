...
- [ ] **Чтение sqfs-образа:**
    ...
    - [ ] Shell-тесты в `tests/08_unfreeze.bats`:
        - [ ] Тест на успешное восстановление.
        - [ ] Тест на восстановление с конфликтами (замена/пропуск).
        - [ ] Тест на восстановление с legacy-форматом `list.yaml`.
        - [ ] Тест ошибку с поврежденным `list.yaml`.
    - [ ] Монтирование образа (через `squash_manager-rs mount`):
        - [ ] В кэше создаём создаём `mount_unixtime_random` (префикс `mount_` вместо `build_`) т.к. там только для чтения меры с блокировкой не нужны.
        - [ ] по завершению unfreeze используем `squash_manager-rs umount` для размонтирования.
    - [ ] Чтение и валидация `list.yaml` из смонтированного образа.
    - [ ] Реализация поддержки legacy-формата `list.yaml`, по отдельности нужны следующие возможности:
        - [ ] Умение распаковывать `original_path` вместо `restore_path` + `name`
        - [ ] Если нету `privilege_mode` спрашиваем у пользователя напрямую что использовать (User или Root), после чего запускаем точно такую же логику как и с `privilege_mode` в `list.yaml`.
    - [ ] Валидация путей с явным запретом на `..` в путях.
- [ ] **Восстановление:**
    - [ ] Логика копирования файлов из `to_restore/<number>` в реальные пути (`original_path` или `restore_path`).
    - [ ] Обработка конфликтов (если файл уже существует) - замена или пропуск (для текущего файла, или для все последующих).
