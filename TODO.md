...
- [ ] **Новый функционал**
    - [x] Внедрить перепаковку архивов вместо работы с обычными каталогами для форматов поверх tar'а: 7z, zip, gzip, bzip2, xz, zstd, rar + сам tar, перепаковку надо делать через пайп, без создания промежуточных файлов
        ...
        - [ ] Рализовать негативные тесты в 04_unpack.bats (нет утилиты, для распаковки, файл повреждён или неизвестный формат)
    - [ ] Внедрить LUKS-шифрование в squash_manager-rs:
        - [ ] Рализовать логику пропуска тестов, требующих прав рута в зависимости от условий (по утверждённому плану)
        - [ ] Реализовать интеграционные shell-тесты для:
            - [ ] Внедрения LUKS-шифрования в squash_manager-rs:
            - [ ] Реализации truncate для сброса неиспользуемого места в конце LUKS-контейнера, используя File::set_len()
            - [ ] Реализация удаления мусора после успешного завершения операции: не должно оставаться маппированных LUKS-контейнеров
            - [ ] Реализация удаления мусора, если операция была прервана: кроме LUKS-контейнера нужно удалить не дописанный до конца squashfs-образ
        - [ ] Реализовать в Rust-коде mock юнит-тесты эмуляции работы с LUKS с правами root, без реального рута
        - [ ] Реализация прохождения всех тестов
...
- [ ] Ручная переработка TODO-листов для этапов 6+
- [ ] **Подготовка данных:**
