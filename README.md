...
**Zero-Kelvin Stazis (zks-rs)** — это высокопроизводительная утилита для выгрузки данных с сохранением их целостности, атрибутов (прав доступа, владельца, временных меток) и расположения. Это порт на Rust оригинального прототипа моих идей изначально реализованных в виде функций `zero-kelvin-store` на Fish shell, предназначенный для «заморозки» проектов и данных в сжатые, монтируемые архивы SquashFS. С возможность опционального шифрования при помощи стандартного `cryptseptup` (LUKS). Основная цель — освобождение дискового пространства при сохранении возможности мгновенного доступа к данным.
