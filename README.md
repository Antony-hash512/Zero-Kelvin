...
**Zero-Kelvin Stazis (zks-rs)** is a high-performance, identity-preserving data offloading utility. It's the Rust port of the original `zero-kelvin-store` Fish functions, embodying my initial ideas for "freezing" projects and data into compressed, mountable SquashFS archives, with optional encryption using standard `cryptsetup` (LUKS). The main goal is to free up disk space while maintaining instant accessibility to your data.
...
**Zero-Kelvin Stazis (zks-rs)** — это высокопроизводительная утилита для выгрузки данных с сохранением их идентичности и атрибутов. Это порт на Rust оригинального прототипа моих идей изначально реализованных в виде функций `zero-kelvin-store` на Fish shell, предназначенный для «заморозки» проектов и данных в сжатые, монтируемые архивы SquashFS. С возможность опционального шифрования при помощи стандартного `cryptseptup` (LUKS). Основная цель — освобождение дискового пространства при сохранении возможности мгновенного доступа к данным.
...
---
### ЧаВо:
В: поддерживается ли дедупликация разных версия бекапов
О: Цель данной утилиты это заморзка и перемещением во внешнее хранение тех данных которые в данный момент не нужны. Для переодический бекапов данных и с которыми вы сейчас работаете, я рекомендую другие уже готовые решения:
...
- Если много бинарных файлов, git подойдёт не очень, тогда рекомендую:
  - Снапшоты в таких файловых системах как btrfs или ZFS (отличное решение для быстрого доступа к старым версия ваших данных даже в бинарном формате, но данное решение не очень подходит для освобождения места в системе путём выгрузки на внешний HDD/NAS)
