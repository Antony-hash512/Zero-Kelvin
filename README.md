...
**Zero-Kelvin Stazis (zks-rs)** — это высокопроизводительная утилита для выгрузки данных с сохранением их целостности, атрибутов (прав доступа, владельцев, временных меток) и расположения. Это порт на Rust оригинального прототипа моих идей изначально реализованных в виде функций `zero-kelvin-store` на Fish shell, предназначенный для «заморозки» проектов и данных в сжатые, монтируемые архивы SquashFS. С возможностью опционального шифрования при помощи стандартного `cryptsetup` (LUKS). Основная цель — освобождение дискового пространства при сохранении возможности мгновенного доступа к данным.
