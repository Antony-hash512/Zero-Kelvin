...
* **Тело сложного цикла** (`for` / `while`), содержащее вложенные `if`/`match`, должно быть вынесено в отдельную вспомогательную функцию (пример: `check_item` вынесена из цикла в `check`, но её можно декомпозировать еще сильнее).
* Группируй передаваемые аргументы в структуры (Context Structs), чтобы не передавать по 10 параметров в каждую функцию.
