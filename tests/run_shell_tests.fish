...
# Функция-обертка для раскраски
function run_colored_bats
    # Запускаем bats и передаем вывод в sed
    # sed заменяет "✓" на "ЗЕЛЕНЫЙ ✓" и "✗" на "КРАСНЫЙ ✗"
    # Флаг -u (unbuffered) обязателен, чтобы вывод шел строку за строкой, а не вис до конца теста
